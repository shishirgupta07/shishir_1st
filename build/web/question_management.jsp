...
<body>
    ...
    <div class="container"> 
                    ...
                    <br>
                      <ul class="nav nav-tabs nav-justified md-tabs" style="background-color: #4b3e8a" id="myTabJust" role="tablist">
                           <li class="nav-item">
                                ...
                                aria-selected="true">view Questions</a>
                          </li>
                            <li class="nav-item">
      ...
      <!-- Editable table -->
                       <div class="card "  >
                                             <h5 style="background-color:#4b3e8a" class="card-header  text-center white-text font-weight-bold text-uppercase py-4 ">View Questions </h5>
                              <div class="card-body">
   ...
   <tbody>
       <tr class="row">
                  <td class="col-lg-3" >
                    <select  class="browser-default" id="course_select" name="">
 ...
 %>
               ...
               %>    
                ...
                <td class="col-lg-3">
                         <select class="browser-default" id="semester_select" name="">
                          <option value="" >select semester</option> 
                         </select>
                </td>
                <td class="col-lg-3">
                     <select class="browser-default" id="subject_select" name="">
                          <option value="" >select subject</option> 
                         </select>
                 </td>
                 <td class="col-lg-3">
                     <select class="browser-default" id="unit_select" name="">
                          <option value="" >select unit</option> 
                         </select>
                 </td>
              </tr>
             <tr id="question_print"> 
             </tr>  
                    </tbody>
 ...
 <div class="tab-pane fade" id="profile-just" role="tabpanel" aria-labelledby="profile-tab-just">
      ...
      <!-- Material form subscription -->
        <div class="card">
                    ...
                    <strong>Add Questions here</strong>
                    </h5>
         <div class="card-body px-lg-5">

  <!-- Form -->      
  <form class="text-center" id="loginform" action="question_add.jsp" style="color: #757575" method="get" >
 ...
 %>
               ...
               <%}%>    
                ...
                <td class="col-lg-3">
                 ...
                 <td class="col-lg-3">
                     <select class="browser-default" id="subjects_select" name="selectSubject">
                          <option value="" >select subject</option> 
                         </select>
                 </td>
                 <td class="col-lg-3">
                     <select class="browser-default" id="units_select" name="selectUnit">
                          <option value="" >select unit</option> 
                         </select>
                 </td>
              </tr>
              <tr>
                  <td>
                      ...
                      <span class="input-group-text md-addon" id="inputGroupMaterial-sizing-default">enter the no of questions you want to enter</span>
                      </div>
                      <input type="text" class="form-control" id="textField" name="questions" aria-label="Sizing example input" aria-describedby="inputGroupMaterial-sizing-default" onkeyup="generate()">
                      </div>
                   </td>
              </tr>
              <tr>
...
<!--    <script type="text/javascript" src="./test_files/compiled.min.js.download"></script>-->
   ...
   <script type="text/javascript">
     ...
     <script type="text/javascript">
    $(document).ready(function(){
         $('#semesters_select').change(function(){
          $('#subjects_select').find('option').not(':first').remove();
                var course_id = $('#courses_select').val();
                var sem_id = $('#semesters_select').val();
                var datastring={course_id:course_id,sem_id:sem_id};
                  $.ajax({
      type:"POST",
      url:"getSubjectOption.jsp",
      ...
      success:function(result)
      {
        ...
        var obj=JSON.parse(result);
       console.log(obj);
       var end= $('#subjects_select').find('option').remove().end();
       end.append('<option value="" selected disabled>Subject</option>');
            for (var i=0;i<obj.length;i++){
       end.append('<option value="'+obj[i].subject_id+'">'+obj[i].subject_name+'</option>');
     }
   ...
   <script type="text/javascript">
    $(document).ready(function(){
      ...
      success:function(result)
      {
        ...
        var obj=JSON.parse(result);
       console.log(obj);
       var end= $('#unit_select').find('option').remove().end();
       end.append('<option value="" selected disabled>Unit</option>');
            for (var i=0;i<obj.length;i++){
       end.append('<option value="'+obj[i].unit_id+'">'+obj[i].unit_name+'</option>');
     }
       }
     });
         });
    ...
    $(document).ready(function(){
         $('#subjects_select').change(function(){
          $('#units_select').find('option').not(':first').remove();
                var subject_id = $('#subjects_select').val();
                var datastring={subject_id:subject_id};
                  $.ajax({
      type:"POST",
      url:"getUnitOption.jsp",
      ...
      success:function(result)
      {
        ...
        var obj=JSON.parse(result);
       console.log(obj);
       var end= $('#units_select').find('option').remove().end();
       end.append('<option value="" selected disabled>Unit</option>');
            for (var i=0;i<obj.length;i++){
       end.append('<option value="'+obj[i].unit_id+'">'+obj[i].unit_name+'</option>');
     }
   ...
   <script>
       function generate(){
            ...
            for (i=1;i<=number;i++){
                ...
                container.appendChild(document.createTextNode("enter question"));
                var input1 = document.createElement("input");
                input1.type = "text";
                input1.name = "questionField"+i;
                container.appendChild(input1);
                // Create an <input> element, set its type and name attributes
                container.appendChild(document.createTextNode("marks"));
                var input2 = document.createElement("input");
                input2.type = "text";
                input2.name = "marksField"+i;
                container.appendChild(input2);
                // Append a line break 
                container.appendChild(document.createTextNode("level"));
                var input3 = document.createElement("input");
                input3.type = "text";
                input3.name = "levelField"+i;
                container.appendChild(input3);
                container.appendChild(document.createElement("br"));
                container.appendChild(document.createElement("br"));
                container.appendChild(document.createTextNode("category"));
                var input4 = document.createElement("input");
                input4.type = "text";
                input4.name = "categoryField"+i;
     ...
     $(document).ready(function(){
         $('#unit_select').change(function(){
          $('#question_print').find('td').remove();
                var subject_id = $('#subject_select').val();
                console.log(subject_id);
                var unit_id = $('#unit_select').val();
                console.log(unit_id);
                var datastring={subject_id:subject_id,unit_id:unit_id};
                  $.ajax({
      type:"POST",
      url:"getQuestion.jsp",
      data:datastring,
      ...
      {
        console.log(result);
        var end= $('#question_print').find('td').remove().end();
       end.append(result);
   }
